...
export function mountWebUI(app, dirname, accountManager) {
  ...
  app.post("/api/config", (req, res) => {
    try {
      ...
      if (accountSelection && typeof accountSelection === "object") {
        const validStrategies = ["sticky", "round-robin", "hybrid", "silent-failover", "on-demand", "aggressive", "quota-first", "conservative"];
  ...
  app.post("/api/claude/config/restore", async (req, res) => {
    try {
      ...
      const PROXY_ENV_VARS = ["ANTHROPIC_BASE_URL", "ANTHROPIC_AUTH_TOKEN", "ANTHROPIC_MODEL", "CLAUDE_CODE_SUBAGENT_MODEL", "ANTHROPIC_DEFAULT_OPUS_MODEL", "ANTHROPIC_DEFAULT_SONNET_MODEL", "ANTHROPIC_DEFAULT_HAIKU_MODEL", "ENABLE_EXPERIMENTAL_MCP_CLI"];
