...
window.Components.accountManager = () => ({
  ...
  get filteredAccounts() {
    ...
    return accounts.filter((acc) => {
      return acc.email.toLowerCase().includes(query) || (acc.projectId && acc.projectId.toLowerCase().includes(query)) || (acc.source && acc.source.toLowerCase().includes(query));
    });
  },
  ...
  async refreshAccount(email) {
    return await window.ErrorHandler.withLoading(
      async () => {
        ...
        const { response, newPassword } = await window.utils.request(`/api/accounts/${encodeURIComponent(email)}/refresh`, { method: "POST" }, store.webuiPassword);
  ...
  async executeDelete() {
    ...
    return await window.ErrorHandler.withLoading(
      async () => {
        ...
        const { response, newPassword } = await window.utils.request(`/api/accounts/${encodeURIComponent(email)}`, { method: "DELETE" }, store.webuiPassword);
        if (newPassword) store.webuiPassword = newPassword;
  ...
  async exportAccounts() {
    const store = Alpine.store("global");
    try {
      const { response, newPassword } = await window.utils.request("/api/accounts/export", {}, store.webuiPassword);
      if (newPassword) store.webuiPassword = newPassword;
  ...
  async regenerateFingerprint(email) {
    return await window.ErrorHandler.withLoading(
      async () => {
        ...
        const { response, newPassword } = await window.utils.request(`/api/accounts/${encodeURIComponent(email)}/fingerprint/regenerate`, { method: "POST" }, store.webuiPassword);
        if (newPassword) store.webuiPassword = newPassword;
      ...
      this,
      "regeneratingFingerprint",
      { errorMessage: "Failed to regenerate fingerprint" },
    );
  },
  ...
  async regenerateAllFingerprints() {
    return await window.ErrorHandler.withLoading(
      async () => {
        ...
        const { response, newPassword } = await window.utils.request("/api/accounts/fingerprints/regenerate-all", { method: "POST" }, store.webuiPassword);
        if (newPassword) store.webuiPassword = newPassword;
      ...
      this,
      "regeneratingAllFingerprints",
      { errorMessage: "Failed to regenerate fingerprints" },
    );
  },
